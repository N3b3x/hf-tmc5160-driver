...
union GCONF_Register {
  ...
  struct {
    uint32_t
        recalibrate : 1; ///< Zero crossing recalibration during driver disable
    uint32_t faststandstill : 1; ///< Timeout for step execution until
                                 ///< standstill detection
    uint32_t en_pwm_mode : 1;    ///< Enable stealthChop voltage PWM mode
    uint32_t
        multistep_filt : 1;   ///< Enable step input filtering for stealthChop
                              ///< optimization with external step source
    uint32_t shaft : 1;       ///< Normal / inverse motor direction
    uint32_t diag0_error : 1; ///< Enable DIAG0 active on driver errors: Over
                              ///< temperature (ot), short to GND (s2g),
                              ///< undervoltage chargepump (uv_cp)
    uint32_t diag0_otpw : 1; ///< Enable DIAG0 active on driver over temperature
                             ///< prewarning (otpw)
    uint32_t
        diag0_stall_step : 1; ///< SD_MODE=1: enable DIAG0 active on motor
                              ///< stall. SD_MODE=0: enable DIAG0 as STEP output
    uint32_t
        diag1_stall_dir : 1; ///< SD_MODE=1: enable DIAG1 active on motor stall.
                             ///< SD_MODE=0: enable DIAG1 as DIR output
    uint32_t diag1_index : 1;   ///< Enable DIAG1 active on index position
    uint32_t diag1_onstate : 1; ///< Enable DIAG1 active when chopper is on
    uint32_t diag1_steps_skipped : 1; ///< Enable output toggle when steps are
                                      ///< skipped in dcStep mode
    ...
    uint32_t small_hysteresis : 1; ///< Set small hysteresis for step frequency
                                   ///< comparison
    uint32_t stop_enable : 1; ///< Enable emergency stop: ENCA_DCIN stops the
                              ///< sequencer when tied high
    uint32_t direct_mode : 1; ///< Enable direct motor coil current and polarity
                              ///< control
    uint32_t test_mode : 1;   ///< Not for normal use
...
union OTP_READ_Register {
  ...
  struct {
    uint32_t otp_fclktrim : 5; ///< Reset default for FCLKTRIM (frequency source
                               ///< calibration)
...
union SHORT_CONF_Register {
  ...
  struct {
    uint32_t s2vs_level : 4;  ///< Short to VS detector for low side FETs
                              ///< sensitivity (4 to 15)
    uint32_t reserved1 : 4;   ///< Reserved bits
    uint32_t s2g_level : 4;   ///< Short to GND detector for high side FETs
                              ///< sensitivity (2 to 15)
    uint32_t reserved2 : 4;   ///< Reserved bits
    uint32_t shortfilter : 2; ///< Spike filtering bandwidth for short detection
                              ///< (0 to 3)
    uint32_t shortdelay : 1;  ///< Short detection delay (0 to 1)
    uint32_t reserved3 : 13;  ///< Reserved bits
  } bits;
};
 ...
 *
 * Driver configuration including break-before-make timing and gate driver
 * strength.
 */
union DRV_CONF_Register {
  ...
  struct {
    ...
    uint32_t bbmclks : 4;   ///< Digital BBM Time in clock cycles (0 to 15)
    uint32_t reserved2 : 4; ///< Reserved bits
    uint32_t otselect : 2;  ///< Selection of over temperature level for bridge
                            ///< disable
    uint32_t drvstrength : 2; ///< Selection of gate drivers current (0 to 3)
    uint32_t filt_isense : 2; ///< Filter time constant of sense amplifier to
                              ///< suppress ringing and coupling from second
                              ///< coil operation
...
union OFFSET_READ_Register {
  ...
  struct {
    uint32_t phase_b : 8;   ///< Phase B offset calibration result
    uint32_t phase_a : 8;   ///< Phase A offset calibration result
    uint32_t reserved : 16; ///< Reserved bits
  } bits;
};
...
union IHOLD_IRUN_Register {
  ...
  struct {
    ...
    uint32_t reserved1 : 3; ///< Reserved bits
    uint32_t irun : 5; ///< Motor run current (0=1/32...31=32/32). Should be
                       ///< between 16 and 31 for best performance.
    uint32_t reserved2 : 3;  ///< Reserved bits
    uint32_t iholddelay : 4; ///< Controls the number of clock cycles for motor
                             ///< power down when entering standstill
...
union ENCMODE_Register {
  ...
  struct {
    uint32_t pol_A : 1; ///< Required A polarity for an N channel event (0=neg.,
                        ///< 1=pos.)
    uint32_t pol_B : 1; ///< Required B polarity for an N channel event (0=neg.,
                        ///< 1=pos.)
    uint32_t pol_N : 1; ///< Defines active polarity of N (0=low active, 1=high
                        ///< active)
    uint32_t ignore_AB : 1; ///< Ignore A and B polarity for N channel event
    uint32_t
        clr_cont : 1; ///< Always latch or latch and clear X_ENC upon an N event
    uint32_t clr_once : 1;    ///< Latch or latch and clear X_ENC on the next N
    ...
    uint32_t
        latch_x_act : 1; ///< Also latch XACTUAL position together with X_ENC
    uint32_t enc_sel_decimal : 1; ///< Encoder prescaler divisor binary mode (0)
                                  ///< / decimal mode (1)
...
union ENC_STATUS_Register {
  ...
  struct {
    ...
    uint32_t
        deviation_warn : 1; ///< Deviation between X_ACTUAL and X_ENC detected
...
union CHOPCONF_Register {
  ...
  struct {
    uint32_t toff : 4; ///< Off time setting controls duration of slow decay
                       ///< phase. 0 : Driver disabled.
    uint32_t hstrt_tfd : 3;   ///< chm=0: hysteresis start value HSTRT, chm=1:
                              ///< fast decay time setting bits 0:2
    uint32_t hend_offset : 4; ///< chm=0: hysteresis low value HEND, chm=1: sine
                              ///< wave offset
    uint32_t tfd_3 : 1;       ///< chm=1: fast decay time setting bit 3
    uint32_t disfdcc : 1;     ///< chm=1: disable current comparator usage for
                              ///< termination of the fast decay cycle
    uint32_t rndtf : 1;       ///< enable random modulation of chopper TOFF time
    uint32_t chm : 1; ///< Chopper mode (0=standard - spreadCycle ; 1=constant
                      ///< off time with fast decay time)
    uint32_t tbl : 2; ///< Comparator blank time select
    uint32_t vsense : 1; ///< Select resistor voltage sensitivity (0=Low
                         ///< sensitivity ; 1=High sensitivity)
    uint32_t
        vhighfs : 1; ///< Enable switching to fullstep when VHIGH is exceeded
    uint32_t vhighchm : 1; ///< Enable switching to chm=1 and fd=0 when VHIGH is
                           ///< exceeded
    uint32_t tpfd : 4;     ///< passive fast decay time
    uint32_t mres : 4;     ///< Set microstep resolution
    uint32_t intpol : 1;   ///< Enable interpolation to 256 microsteps with an
...
union COOLCONF_Register {
  ...
  struct {
    uint32_t semin : 4; ///< Minimum stallGuard2 value for smart current control
                        ///< and smart current enable
    uint32_t reserved1 : 1; ///< Reserved bits
    uint32_t seup : 2;      ///< Current increment step width
    uint32_t reserved2 : 1; ///< Reserved bits
    uint32_t
        semax : 4; ///< stallGuard2 hysteresis value for smart current control
    ...
    uint32_t reserved5 : 7; ///< Reserved bits
  } bits;
};
...
union DRV_STATUS_Register {
  ...
  struct {
    uint32_t sg_result : 9;  ///< stallGuard2 result or motor temperature
                             ///< estimation in stand still
    uint32_t reserved1 : 3;  ///< Reserved bits
...
union PWMCONF_Register {
  ...
  struct {
    ...
    uint32_t freewheel : 2; ///< Stand still option when motor current setting
                            ///< is zero (I_HOLD=0)
...
union PWM_SCALE_Register {
  ...
  struct {
    ...
    uint32_t pwm_scale_auto : 9; ///< Result of the automatic amplitude
