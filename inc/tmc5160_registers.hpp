...
constexpr uint8_t TPWMTHRS =
    0x13; ///< Upper velocity for stealthChop voltage PWM mode
constexpr uint8_t TCOOLTHRS =
    0x14; ///< Lower threshold velocity for switching on smart energy coolStep
          ///< and stallGuard feature
...
constexpr uint8_t RAMPMODE =
    0x20; ///< Driving mode (Velocity, Positioning, Hold)
...
constexpr uint8_t V_1 =
    0x25; ///< First acceleration/deceleration phase target velocity
...
constexpr uint8_t TZEROWAIT =
    0x2C; ///< Waiting time after ramping down to zero velocity before next
...
constexpr uint8_t VDCMIN =
    0x33; ///< Velocity threshold for enabling automatic commutation dcStep
...
constexpr uint8_t XLATCH =
    0x36; ///< Ramp generator latch position upon programmable switch event

// Encoder registers
constexpr uint8_t ENCMODE =
    0x38;                       ///< Encoder configuration and use of N channel
...
constexpr uint8_t ENC_DEVIATION =
    0x3D; ///< Maximum number of steps deviation between encoder counter and
          ///< XACTUAL for deviation warning
...
constexpr uint8_t MSLUTSTART =
    0x69; ///< Absolute current at microstep table entries 0 and 256
...
constexpr uint8_t DCCTRL =
    0x6E; ///< dcStep automatic commutation configuration register
constexpr uint8_t DRV_STATUS =
    0x6F; ///< stallGuard2 value and driver error flags
constexpr uint8_t PWMCONF =
    0x70; ///< stealthChop voltage PWM mode chopper configuration
constexpr uint8_t PWM_SCALE =
    0x71; ///< Results of stealthChop amplitude regulator
constexpr uint8_t PWM_AUTO =
    0x72; ///< Automatically determined PWM config values
 ...
 * - Bit 7: diag0_stall - (with SD_MODE=1) 1: Enable DIAG0 active on motor stall (set TCOOLTHRS before using this feature)
 *          diag0_step - (with SD_MODE=0) 0: DIAG0 outputs interrupt signal, 1: Enable DIAG0 as STEP output (half frequency, dual edge triggered) for external STEP/DIR driver
 * - Bit 8: diag1_stall - (with SD_MODE=1) 1: Enable DIAG1 active on motor stall (set TCOOLTHRS before using this feature)
...
union GCONF_Register {
  ...
  struct {
    uint32_t recalibrate : 1; ///< Bit 0: 1=Zero crossing recalibration during driver disable (via DRV_ENN or via TOFF setting)
    uint32_t faststandstill : 1; ///< Bit 1: Standstill timeout: 1=Short time (2^18 clocks), 0=Normal time (2^20 clocks)
    uint32_t en_pwm_mode : 1; ///< Bit 2: 1=StealthChop voltage PWM mode enabled (depending on velocity thresholds). Switch only in stand-still at IHOLD=nominal IRUN.
    uint32_t multistep_filt : 1; ///< Bit 3: 1=Enable step input filtering for StealthChop optimization with external step source (default=1)
    uint32_t shaft : 1; ///< Bit 4: 1=Inverse motor direction
    uint32_t diag0_error : 1; ///< Bit 5: (only with SD_MODE=1) 1=Enable DIAG0 active on driver errors (OT, S2G, UV_CP). DIAG0 always shows reset-status.
    uint32_t diag0_otpw : 1; ///< Bit 6: (only with SD_MODE=1) 1=Enable DIAG0 active on driver over temperature prewarning (otpw)
    uint32_t diag0_stall_step : 1; ///< Bit 7: (SD_MODE=1) 1=Enable DIAG0 active on motor stall (set TCOOLTHRS before using). (SD_MODE=0) 0=DIAG0 outputs interrupt signal, 1=Enable DIAG0 as STEP output (half frequency, dual edge triggered)
    uint32_t diag1_stall_dir : 1; ///< Bit 8: (SD_MODE=1) 1=Enable DIAG1 active on motor stall (set TCOOLTHRS before using). (SD_MODE=0) 0=DIAG1 outputs position compare signal, 1=Enable DIAG1 as DIR output
    uint32_t diag1_index : 1; ///< Bit 9: (only with SD_MODE=1) 1=Enable DIAG1 active on index position (microstep look up table position 0)
    uint32_t diag1_onstate : 1; ///< Bit 10: (only with SD_MODE=1) 1=Enable DIAG1 active when chopper is on (for the coil which is in the second half of the fullstep)
    uint32_t diag1_steps_skipped : 1; ///< Bit 11: (only with SD_MODE=1) 1=Enable output toggle when steps are skipped in DcStep mode (increment of LOST_STEPS). Do not enable in conjunction with other DIAG1 options.
    uint32_t diag0_int_pushpull : 1; ///< Bit 12: SWN_DIAG0 output: 0=open collector (active low), 1=push pull (active high)
    uint32_t diag1_poscomp_pushpull : 1; ///< Bit 13: SWP_DIAG1 output: 0=open collector (active low), 1=push pull (active high)
    uint32_t small_hysteresis : 1; ///< Bit 14: Step frequency comparison hysteresis: 0=1/16, 1=1/32
    uint32_t stop_enable : 1; ///< Bit 15: 0=Normal operation, 1=Emergency stop: ENCA_DCIN stops the sequencer when tied high (no steps become executed, motor goes to standstill)
    uint32_t direct_mode : 1; ///< Bit 16: 0=Normal operation, 1=Direct motor coil control: XTARGET bits 8..0=coil A current, bits 24..16=coil B current (scaled by IHOLD). Velocity based current regulation of StealthChop not available.
    uint32_t test_mode : 1; ///< Bit 17: 0=Normal operation, 1=Enable analog test output on pin ENCN_DCO. IHOLD[1..0] selects function (0..2: T120, DAC, VDDH). Hint: Not for user, set to 0!
...
union OTP_PROG_Register {
  ...
  struct {
    uint32_t otpbit : 3;     ///< Bits 2..0: Selection of OTP bit to be programmed (n=0..7: programs bit n to logic 1)
...
union SHORT_CONF_Register {
  ...
  struct {
    uint32_t s2vs_level : 4;  ///< Bits 3..0: Short to VS detector for low side FETs (4-15, highest to lowest sensitivity)
    uint32_t reserved1 : 4;   ///< Bits 7..4: Reserved
    uint32_t s2g_level : 4;   ///< Bits 11..8: Short to GND detector for high side FETs (2-15, highest to lowest sensitivity)
...
union DRV_CONF_Register {
  ...
  struct {
    uint32_t bbmtime : 5;    ///< Break-Before-Make delay (0..24): 0=100ns ... 16=200ns ... 24=375ns. >24 not recommended, use BBMCLKS instead.
    uint32_t reserved1 : 3;  ///< Reserved
    uint32_t bbmclks : 4;    ///< Digital BBM time in clock cycles (0..15). Each step ≈83ns. Longer of BBMTIME or BBMCLKS prevails.
    uint32_t reserved2 : 4;  ///< Reserved
    uint32_t otselect : 2;   ///< Overtemperature (OTPW) bridge disable threshold: 0=150°C, 1=143°C, 2=136°C, 3=120°C
...
union ENCMODE_Register {
  ...
  struct {
    ...
    uint32_t clr_once : 1; ///< Bit 5: Latch or latch and clear X_ENC on the next N event following the write access
 ...
 * 
 * Bit assignments per datasheet:
 * - Bit 0: n_event - Event detected. To clear the status bit, write with a 1 bit at the corresponding position.
 *   This bit is ORed to the interrupt output signal.
 * - Bit 1: deviation_warn - Deviation warning. Deviation_warn cannot be cleared while a warning still persists.
 *   Set ENC_DEVIATION zero to disable. This bit is ORed to the interrupt output signal.
 */
union ENC_STATUS_Register {
  ...
  struct {
    ...
    uint32_t deviation_warn : 1; ///< Bit 1: Deviation between X_ACTUAL and X_ENC detected (Write '1' to clear, but only if warning no longer persists)
 ...
 *   %0010...%1111: 2 ... 15
 * - Bits 6..4: hstrt2..hstrt0/TFD[2..0] - Hysteresis start value (chm=0) or fast decay time setting (chm=1) (3 bits)
 *   chm=0: %000...%111: Add 1, 2, ..., 8 to hysteresis low value HEND
 *   chm=1: Fast decay time setting TFD (MSB: fd3)
 * - Bits 10..7: hend3..hend0/HEND/OFFSET - Hysteresis low value (chm=0) or sine wave offset (chm=1) (4 bits)
 *   chm=0: %0000...%1111: Hysteresis is -3, -2, -1, 0, 1, ..., 12
 ...
 * 
 * Bit assignments per datasheet:
 * - Bits 3..0: semin3..semin0 - Minimum StallGuard2 value for smart current control and smart current enable (4 bits)
 ...
 *   Mechanical load measurement: Higher value means lower mechanical load. Value of 0 signals highest load.
 *   Temperature measurement: In standstill, shows chopper on-time for motor coil A.
...
union DRV_STATUS_Register {
  ...
  struct {
    ...
    uint32_t reserved2 : 3; ///< Bits 23..21: Reserved, ignore these bits (bit 21 is also reserved per datasheet)
 ...
 * - Bits 21..20: freewheel1..freewheel0 - Stand still option when motor current setting is zero (I_HOLD=0) (2 bits)
 *   %00: Normal operation
 *   %01: Freewheeling
...
union PWM_SCALE_Register {
  ...
  struct {
    ...
    int32_t pwm_scale_auto : 9; ///< Bits 24..16: Result of automatic amplitude regulation (signed -255...+255)
    uint32_t reserved2 : 7;      ///< Reserved bits (25..31)
  } bits;
};
