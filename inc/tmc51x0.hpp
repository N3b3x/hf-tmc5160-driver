/**
 * @file tmc51x0.hpp
 * @brief Main TMC51x0 stepper motor driver interface and subsystem classes (TMC5130 & TMC5160)
 *
...
/**
 * @brief Main class representing a TMC51x0 stepper motor driver (TMC5130 & TMC5160)
 * @ingroup TMC51X0_Core
 *
 * The TMC51x0 class provides a comprehensive high-level interface for
 * configuring and controlling TMC5130 and TMC5160 stepper motor driver chips. This class
...
public:
   ...
   *
   * @note The clock frequency (f_clk) is determined during Initialize() from DriverConfig::external_clk_config:
   *       - If external_clk_config.frequency_hz = 0: Uses internal oscillator (~12 MHz, CLK pin tied to GND)
   *       - If external_clk_config.frequency_hz > 0: Uses external clock at specified frequency (CLK pin receives clock
   * signal)
   *       - The internal clock has ±4% velocity precision and includes a watchdog that switches back to internal clock
   ...
   * This includes the initial configuration plus any modifications made after initialization.
   *
   * @note If the driver is not initialized, the returned config will contain default values.
   */
   ...
   * @param initialized Reference to store initialization status (true if initialized, false otherwise)
   * @return Current driver configuration (including all runtime changes)
   *
   * Retrieves the current driver configuration including all runtime changes.
   ...
   * @return String containing complete driver configuration and status information
   *
   * Returns a human-readable string with all driver configuration, register values,
   * and current status. Useful for debugging and diagnostics.
   *
   * @note The returned string is allocated on the heap. Caller is responsible for freeing.
   * @note For embedded systems, consider using a fixed-size buffer version instead.
   */
  ...
  struct RampControl {
     ...
     *
     * @note Used only with the internal ramp generator (positioning / velocity mode).
  ...
  struct MotorControl {
     ...
     * Above this threshold, SpreadCycle is used (higher torque, more noise).
     *
     * Setting to 0.0 disables the threshold (StealthChop always used if enabled).
     *
     ...
     *
     * Current is scaled by IHOLD setting. The STEP/DIR inputs and motion controller
     * are not used in direct mode.
     *
     * @note Requires direct_mode to be enabled via ConfigureGlobalConfig().
     * @note For SpreadCycle mode, limit amplitude to ±248 for IHOLD up to 31 and
     *       hysteresis settings not exceeding 15 to keep sufficient regulation margin.
     ...
     *
     * The delay is automatically calculated from the desired total delay time, clock frequency,
     * and the number of current reduction steps (IRUN - IHOLD).
     *
     * Calculation: IHOLDDELAY = round((total_delay_ms / (IRUN - IHOLD)) × f_clk / (1000 × 2^18))
     * Total delay = (IRUN - IHOLD) × IHOLDDELAY × (2^18 / f_clk)
     *
     * @note If IRUN == IHOLD, the delay is always 0 regardless of this setting.
     * @note At 12 MHz with 15 current steps: typical range is 200-700 ms total delay.
  ...
  struct Communication {
     ...
     * If config.frequency_hz > 0, uses external clock at specified frequency.
     *
     * This method:
     ...
     *
     * @note This is automatically called during Initialize() with the value from DriverConfig.
     * @note If SetClkFreq() is not supported (returns false), CLK pin should be tied to GND for internal oscillator.
     ...
     *       (using ConfigureUartNodeAddress() or uartConfig.ConfigureUartNodeAddress()).
     *       This method only updates the software representation.
     *       Only applicable for UART communication interfaces.
     */
    void SetUartNodeAddress(uint8_t address) noexcept {
      driver_.uart_node_address_ = address & 0xFF; // Address range is 0-254 (8-bit)
    }
     ...
     * (SPI/UART/Standalone) and the motion control method (Internal ramp generator vs External step/dir).
     *
     * ⚠️ CRITICAL WARNINGS:
  ...
  struct Diagnostics {
     ...
     * Setting to 0 disables these features at all speeds.
     *
  ...
  struct Tuning {
    ...
    /**
     * @brief Automatically tune StallGuard threshold (SGT) with comprehensive velocity range analysis
     ...
     * @param min_velocity Minimum velocity to verify tuning at (0 = disabled, used to determine SGT range)
     ...
     * @note Min and max velocities are used to determine the range of SGT values that work
     ...
     *
     * This is a convenience wrapper around the new comprehensive tuning function.
  ...
  struct Homing {
    ...
    /**
     * @brief Perform sensorless homing using StallGuard2 (with settings caching)
   ...
   * @ingroup TMC51X0_Subsystems
   * 
   * Prints register values using the communication interface's LogDebug() method.
   * All methods use TMC51X0_LOG_DEBUG() macro which respects TMC51X0_DISABLE_DEBUG_LOGGING.
   */
  struct Printer {
    /**
...
private:
  ...
  uint8_t chip_version_{ChipVersion::TMC5160};        ///< Detected chip version (0x11 = TMC5130, 0x30 = TMC5160)
  ...
  // Represents the current runtime state, updated whenever any Configure* method is called
