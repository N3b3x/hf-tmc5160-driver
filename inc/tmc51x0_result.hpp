...
inline const char* ErrorMessage(ErrorCode code) {
    switch (code) {
        ...
        case ErrorCode::NOT_INITIALIZED: return "Driver not initialized";
        case ErrorCode::COMM_ERROR: return "Communication error";
        case ErrorCode::INVALID_VALUE: return "Invalid parameter value";
        case ErrorCode::INVALID_STATE: return "Invalid state for operation";
        case ErrorCode::TIMEOUT: return "Operation timed out";
        case ErrorCode::HARDWARE_ERROR: return "Hardware fault detected";
        case ErrorCode::SHORT_CIRCUIT: return "Short circuit detected";
        case ErrorCode::OPEN_LOAD: return "Open load detected";
        case ErrorCode::OVERTEMP_WARNING: return "Overtemperature warning";
        case ErrorCode::OVERTEMP_SHUTDOWN: return "Overtemperature shutdown";
        case ErrorCode::UNSUPPORTED: return "Feature not supported";
...
public:
    ...
    decltype(auto) get() const noexcept {
        if constexpr (N == 0) return error_;
...
namespace std {
    ...
    template<typename T>
    struct tuple_element<0, tmc51x0::Result<T>> {
        using type = tmc51x0::ErrorCode;
    };
    
    template<typename T>
    struct tuple_element<1, tmc51x0::Result<T>> {
        using type = T;
    };
}
