 ...
 * management for different hardware implementations. Supports both TMC5130 and
 * TMC5160 chips.
 *
 ...
 * - Bit positions: bit 39 (MSB, transmitted first) ... bit 0 (LSB, transmitted
 * last)
 ...
 *   - DCEN, DCIN, DCO: DC Step control pins (same physical pins as ENCB, ENCA,
 * ENCN)
 *   - DIAG0, DIAG1: Diagnostic outputs
 * - SPI_MODE=0, SD_MODE=0: UART single wire interface mode
 *   - DIAG1_SWP (pin 27): Single wire I/O (positive) - Use SWP alone for single
 * wire UART
 *   - DIAG0_SWN (pin 26): Single wire I/O (negative) - Use SWP+SWN for RS485
 * differential bus
 ...
 *   - CSN_CFG3, SCK_CFG2, ENCB_CFG4, ENCA_CFG5: Configuration inputs (CFG
 * functions)
...
enum class TMC51x0CtrlPin : uint8_t {
  ...
  DIR,  ///< Direction pin (REFR_DIR, pin 18) - DIR input when SD_MODE=1, Right
        ///< reference when SD_MODE=0
  STEP, ///< Step pin (REFL_STEP, pin 17) - STEP input when SD_MODE=1, Left
        ///< reference when SD_MODE=0
  ...
  REFL_STEP, ///< Left reference switch input (REFL_STEP, pin 17) - Used when
             ///< SD_MODE=0
  REFR_DIR,  ///< Right reference switch input (REFR_DIR, pin 18) - Used when
             ///< SD_MODE=0

  // Diagnostic output pins / UART pins (mode-dependent)
  DIAG0, ///< Diagnostic output 0 (DIAG0_SWN, pin 26) - Interrupt/STEP output
         ///< when SD_MODE=0, SPI_MODE=1.
  ///< Single wire I/O (negative) when SD_MODE=0, SPI_MODE=0 (UART mode). Use
  ///< with DIAG1_SWP for RS485 bus.
  DIAG1, ///< Diagnostic output 1 (DIAG1_SWP, pin 27) - Position-compare/DIR
         ///< output when SD_MODE=0, SPI_MODE=1.
  ///< Single wire I/O (positive) when SD_MODE=0, SPI_MODE=0 (UART mode). Use
  ///< alone for single wire UART.
  ...
  ENCA, ///< Encoder A-channel input (ENCA_DCIN_CFG5, pin 24) - Used when
        ///< SD_MODE=0
  ENCB, ///< Encoder B-channel input (ENCB_DCEN_CFG4, pin 23) - Used when
        ///< SD_MODE=0
  ENCN, ///< Encoder N-channel input (ENCN_DCO_CFG6, pin 25) - Used when
        ///< SD_MODE=0
  ...
  DCEN, ///< DC Step enable input (ENCB_DCEN_CFG4, pin 23) - Used when
        ///< SD_MODE=1, SPI_MODE=1
  DCIN, ///< DC Step gating input (ENCA_DCIN_CFG5, pin 24) - Used when
        ///< SD_MODE=1, SPI_MODE=1
  DCO,  ///< DC Step ready output (ENCN_DCO_CFG6, pin 25) - Used when SD_MODE=1,
        ///< SPI_MODE=1

  // Clock pin (optional external clock)
  CLK, ///< Clock input (CLK, pin 12) - External clock input (tie to GND for
       ///< internal clock)
  ...
  SPI_MODE, ///< SPI/UART mode select (pin 22) - HIGH=SPI, LOW=UART. Typically
            ///< hardwired.
  SD_MODE   ///< Step/Dir mode select (pin 21) - HIGH=External step/dir,
            ///< LOW=Internal ramp. Typically hardwired.
};
 ...
 *
 * Users can create an instance of this struct, modify specific pin active
 * levels if their board has inverters, NOT gates, or other logic that changes
 * pin polarity, and pass it to the communication interface constructor.
 *
 * Example usage:
 * @code
 ...
 * @endcode
 *
 * @note Platform-specific pin configuration structures (like Esp32SpiPinConfig)
 * are defined in platform-specific implementation files, not in this core
 * interface.
 */
struct PinActiveLevels {
  ...
  bool ref_left{
      false}; ///< REFL_STEP (pin 17): LOW=active (typically active LOW)
  bool ref_right{
      false}; ///< REFR_DIR (pin 18): LOW=active (typically active LOW)

  // Diagnostic pins (read-only outputs from TMC51x0 - not typically configured)
  bool diag0{true}; ///< DIAG0 (pin 26): Read-only output (default HIGH, but not
                    ///< used for control)
  bool diag1{true}; ///< DIAG1 (pin 27): Read-only output (default HIGH, but not
                    ///< used for control)

  // Encoder pins (when SD_MODE=0, read-only inputs)
  bool enca{true}; ///< ENCA (pin 24): Read-only input (default HIGH, but not
                   ///< used for control)
  bool encb{true}; ///< ENCB (pin 23): Read-only input (default HIGH, but not
                   ///< used for control)
  bool encn{true}; ///< ENCN (pin 25): Read-only input (default HIGH, but not
                   ///< used for control)
  ...
  bool dco{true};  ///< DCO (pin 25): Read-only output (default HIGH, but not
                   ///< used for control)

  // Clock pin
  bool clk{
      true}; ///< CLK (pin 12): HIGH=active (active HIGH, if used as output)

  // Mode configuration pins (if available as control pins)
  bool spi_mode{true}; ///< SPI_MODE (pin 22): HIGH=SPI mode (active HIGH)
  bool sd_mode{
      true}; ///< SD_MODE (pin 21): HIGH=External Step/Dir (active HIGH)
 ...
 * - Pin 17: REFL_STEP (SD_MODE=0) or STEP (SD_MODE=1) - specify via step_pin or
 * ref_left_pin
 * - Pin 18: REFR_DIR (SD_MODE=0) or DIR (SD_MODE=1) - specify via dir_pin or
 * ref_right_pin
 * - Pin 23: ENCB (SD_MODE=0) or DCEN (SD_MODE=1) - specify via enc_b_pin or
 * dc_en_pin
 * - Pin 24: ENCA (SD_MODE=0) or DCIN (SD_MODE=1) - specify via enc_a_pin or
 * dc_in_pin
 * - Pin 25: ENCN (SD_MODE=0) or DCO (SD_MODE=1) - specify via enc_n_pin or
 * dc_out_pin
 *
...
struct TMC51x0PinConfig {
  ...
  int dir_pin{
      -1}; ///< DIR pin (REFR_DIR, pin 18) - Optional, same as ref_right_pin
  int step_pin{
      -1}; ///< STEP pin (REFL_STEP, pin 17) - Optional, same as ref_left_pin

  // Reference switch pins (when SD_MODE=0)
  int ref_left_pin{
      -1}; ///< Left reference switch (REFL_STEP, pin 17) - Same as step_pin
  int ref_right_pin{
      -1}; ///< Right reference switch (REFR_DIR, pin 18) - Same as dir_pin
  ...
  // DC Step pins (when SD_MODE=1, SPI_MODE=1)
  int dc_in_pin{-1}; ///< DC Step gating input (ENCA_DCIN_CFG5, pin 24) - Same
                     ///< as enc_a_pin
  int dc_en_pin{-1}; ///< DC Step enable input (ENCB_DCEN_CFG4, pin 23) - Same
                     ///< as enc_b_pin
  int dc_out_pin{
      -1}; ///< DC Step ready output (ENCN_DCO_CFG6, pin 25) - Same as enc_n_pin
  ...
  // a chip reset.
  int spi_mode_pin{-1}; ///< SPI_MODE pin (pin 22) - Optional, typically
                        ///< hardwired. HIGH=SPI, LOW=UART
  int sd_mode_pin{-1};  ///< SD_MODE pin (pin 21) - Optional, typically
...
struct SpiStatus {
   ...
   * @return true if driver_error is set (reset_flag is informational, not an
   * error)
   *
   * Note: Reset flag indicates the chip was reset (normal on power-up) and is
   * cleared when read. Only driver_error indicates an actual error condition.
   */
 ...
 *
 * Represents a standard TMC5160 SPI command (40 bits: 8-bit address + 32-bit
 * data). Used for both single-chip and multi-chip (daisy-chain) communication.
...
struct SpiCommand {
  ...
  union Frame {
    ...
    struct {
      uint8_t
          address_byte; ///< Address byte (bit 7 = write, bits 6-0 = address)
      uint8_t data_bytes[4]; ///< Data bytes (MSB to LSB)
    } fields;                ///< Frame as structured fields
    uint64_t raw; ///< Frame as 64-bit value (for easy initialization, upper 24
                  ///< bits unused)
 ...
 *
 * TMC5160 uses CRC8-ATM polynomial (x^8 + x^2 + x^1 + x^0 = 0x07) with initial
 * value of zero. The CRC is applied LSB to MSB, including the sync- and
 * addressing byte. The sync nibble is assumed to always be correct.
 *
 ...
 * Supports write access (8 bytes), read request (4 bytes), and read reply (8
 * bytes). Automatically calculates and verifies CRC8 checksum.
 *
 * Frame Structure (per datasheet section 5.1):
 * - Write Access: Byte 0 (sync+rsv), Byte 1 (nodeaddr), Byte 2 (RW+addr), Bytes
 * 3-6 (data), Byte 7 (CRC)
 * - Read Request: Byte 0 (sync+rsv), Byte 1 (nodeaddr), Byte 2 (RW+addr), Byte
 * 3 (CRC)
 * - Read Reply: Byte 0 (sync+rsv), Byte 1 (0xFF), Byte 2 (addr=0), Bytes 3-6
 * (data), Byte 7 (CRC)
 *
...
public:
   ...
   * @param frequency_hz Desired clock frequency in Hz (0 = use internal clock,
   * >0 = external clock frequency)
   ...
   * method will return false, indicating that the internal oscillator should be
   * used.
   *
   ...
   *   - Return true if CLK pin was successfully set to GND, false if not
   * supported
   *   - f_clk in DriverConfig should still be set correctly (typically 12000000
   * Hz)
   *   - The driver uses f_clk for timing calculations regardless of clock
   * source
   *
   * - **External Clock**:
   ...
   *
   * **Important:**
   * - The f_clk value in DriverConfig is used for all timing calculations
   * (IHOLDDELAY, TPOWERDOWN, TZEROWAIT, etc.)
   * - f_clk must be set correctly regardless of whether using internal or
   * external clock
   ...
   * - Passing `frequency_hz = 0` allows users with external clock capability to
   * switch back to internal clock
   *
   * **Implementation Guidelines:**
   * - If your system doesn't support clock control, return
   * ErrorCode::UNSUPPORTED (driver will assume internal clock)
   * - If your system supports clock control:
   *   - When `frequency_hz = 0`: Set CLK pin to GND (low) and return
   * Result<void>()
   *   - When `frequency_hz > 0`: Provide clock signal at specified frequency
   * and return Result<void>()
   *   - Return error only if the operation failed (e.g., invalid frequency,
   * hardware error)
   *
   * @note This is called automatically during Initialize() with the f_clk value
   * from DriverConfig.
   * @note If not implemented (returns UNSUPPORTED), the driver assumes internal
   * clock (CLK pin tied to GND).
   * @note The internal clock has a fail-over circuit that protects against loss
   * of external clock signal.
   * @note Per datasheet: "Tie to GND using short wire for internal clock or
 ...
 *
 * Uses a 4-wire SPI bus (mode 3) to exchange 40-bit datagrams per datasheet
 * section 4.1. The implementation uses 8 bytes (64 bits) for convenience, which
 * is acceptable per datasheet as additional bits beyond 40 are shifted through
 * an internal shift register for daisy-chaining. Data is sent MSB-first,
 * big-endian.
 *
 ...
 * - CSN: Chip select input (active low, must stay low during entire
 * transaction)
 *
 ...
 *
 * The TMC5160 supports daisy-chaining multiple chips on a single SPI bus per
 * datasheet section 4.2. Each chip has an internal 40-bit shift register. Data
 * on SDI is continuously shifted out on SDO with a 40-clock cycle delay,
 * allowing data to propagate through the chain.
 *
 * Hardware Setup for Daisy-Chaining:
 * - All chips share: CSN (tied together), SCK, and SDI (MOSI)
 * - MISO is daisy-chained: Master -> Chip0 SDO -> Chip1 SDI, Chip1 SDO -> Chip2
 * SDI, etc.
 * - Last chip's SDO connects back to Master MISO
 * - CS must be held low during the entire transfer (all bits must be sent while
 * CS is low)
 *
 * How Daisy-Chaining Works:
 * - Data shifts continuously through the chain as long as CS is low and SCK is
 * active
 * - Each chip's SDO outputs data with a 40-clock delay (data clocked into SDI
 * appears on SDO after 40 clock cycles)
 * - Commands are latched when CS goes HIGH (rising edge) - each chip latches
 * the 40 bits currently in its internal shift register at that moment
 * - To address chip k (where k=0 is first chip, k=1 is second chip, etc.):
 *   - Send 40 bits of command (first)
 *   - Then send 40·k bits of padding (dummy zeros) to shift the command to chip
 * k
 *   - Total bits = 40·(k+1)
 *   - The command for chip k must be the FIRST 40 bits sent, which will
 * propagate to chip k's shift register after (k+1)*40 clock cycles
 *
 * Response Reading (per datasheet):
 * - Response from chip k in a chain of n devices requires sending 40·(n-k)
 * dummy bits total Formula: 40·(n-k) bits of padding + 40 bits of command =
 * 40·(n-k) bits (command included in total)
 * - **CRITICAL**: Responses come back in REVERSE order (last device first,
 * first device last)
 ...
 *   - Extra bytes beyond (k+1)*5 are padding (zeros) for full-duplex response
 * extraction
 *
 * **Chain Length Requirements:**
 * - Chain length MUST be known for correct response extraction
 * - If `daisy_chain_position > 0` and chain length is unknown, it is
 ...
 * - If `daisy_chain_position > 0` and chain length is unknown, it is
 * automatically detected on the first ReadRegister() or WriteRegister() call
 * - Detection uses `AutoDetectChainLength()` which sends a unique command that
 * loops back
 ...
 * - If user calls `SetDaisyChainLength(n)`, the specified length is verified
 * against auto-detected length on first access
 * - If mismatch is detected, an error is logged: "DAISY CHAIN LENGTH MISMATCH!"
 * - The detected length is used (not the user-specified value) to ensure
 * correctness
 *
 * **Why This Matters:**
 ...
 * - Auto-detection ensures correctness without requiring user to manually
 * specify length
 * - Verification catches configuration errors early
 *
 * For multi-chip simultaneous communication:
 * - Commands must be sent in REVERSE order: [cmd_n-1] [cmd_n-2] ... [cmd_0]
 * (last device first)
 ...
 *   - After n*40 clocks total, device 0 has cmd_0, device 1 has cmd_1, ...,
 * device n-1 has cmd_n-1
 * - When CS goes HIGH (rising edge), each device latches the 40 bits currently
 * in its shift register
 * - The reverse order ensures that after n*40 clocks, each device has the
 * correct command to latch Example for 3-chip daisy chain (n=3, devices 0, 1,
 * 2):
 * - Single chip addressing:
 *   - Address chip 0 (k=0): Send 40-bit command + 0 padding = 40 bits (5 bytes)
 *   - Address chip 1 (k=1): Send 40-bit command + 40 padding = 80 bits (10
 * bytes)
 *   - Address chip 2 (k=2): Send 40-bit command + 80 padding = 120 bits (15
 * bytes)
 * - Multi-chip simultaneous (all 3 chips):
 *   - Send: [cmd_2] [cmd_1] [cmd_0] = 120 bits (15 bytes, back-to-back, reverse
 * order)
 *   - As data shifts continuously through the chain (CS held low):
 *     - After 40 clocks: Device 0 has cmd_2 in shift register, Device 1 has
 * nothing, Device 2 has nothing
 *     - After 80 clocks: Device 0 has cmd_1 (shifted in), Device 1 has cmd_2
 * (from Device 0's SDO), Device 2 has nothing
 *     - After 120 clocks: Device 0 has cmd_0 (shifted in), Device 1 has cmd_1
 * (from Device 0's SDO), Device 2 has cmd_2 (from Device 1's SDO)
 *   - When CS goes HIGH (rising edge): Each device latches the 40 bits
 * currently in its shift register
 *     - Device 0 latches cmd_0 ✓
 *     - Device 1 latches cmd_1 ✓
 *     - Device 2 latches cmd_2 ✓
 * - Read response from chip k: Send 40·(n-k) dummy bits total (40·(n-k-1)
 * padding + 40 command, or just 40 command if k=n-1) Response appears at offset
 * (n-k-1)*5 bytes (reverse order: last device first)
 *   **CRITICAL**: Responses come back in REVERSE order - device n-1 response
 * first, then n-2, ..., device 0 last
 *
 * Architecture:
 * - Each SpiCommInterface instance supports ONE SPI bus (shared by multiple
 * TMC5160 drivers)
 * - For daisy-chaining: Each TMC5160 instance tracks its own position and
 * passes it to ReadRegister()/WriteRegister()
 * - For multi-CS setups: Create separate SpiCommInterface instances (one per
 * chip)
 * - ReadRegister() and WriteRegister() automatically handle daisy-chaining
 * based on position parameter
 * - SetDaisyChainLength() must be called to enable optimal response extraction
 * using datasheet formula
 * - For higher-level multi-driver management, use TMC51x0DaisyChain class that
 * manages multiple TMC5160 instances on a single SPI bus and automatically
 * configures chain length
 *
 * Example usage:
 * @code
 * class MySPI : public tmc5160::SpiCommInterface<MySPI> {
...
public:
   ...
   * @param total_length Total number of devices in the chain (1 = single chip,
   * 2 = two chips, etc.) Set to 0 to disable daisy-chain mode (single chip,
   * default)
   * @note This is CRITICAL for proper response extraction using the datasheet
   * formula 40·(n-k)
   * @note This should be set once during initialization, before any register
   * access
   * @note For optimal efficiency with TMC51x0DaisyChain, set this to match the
   * chain length
   * @note If set to a non-zero value, it will be verified against auto-detected
   * length on first access. If mismatch is detected, ReadRegister/WriteRegister
   * will return false.
   * @note If set to 0 and daisy_chain_position > 0, chain length will be
   * auto-detected on first access.
   */
   ...
   * @return Total chain length (0 = unknown/single chip, >0 = total number of
   * devices)
   */
  ...
  /**
   * @brief Auto-detect the daisy chain length by sending a unique command that
   * loops back
   *
   * This method sends a command with a unique, recognizable pattern to position
   * (max_devices+1), which is beyond the last device. The command will shift
   * through all devices and loop back to the MCU via the last device's SDO. By
   * searching for our exact command pattern in the received data, we can
   * determine the actual chain length.
   *
   * Algorithm:
   * 1. Create a unique command with a distinctive pattern (e.g., read register
   * 0x73 with unique data)
   * 2. Send command to position (max_devices+1) - beyond the last device
   * 3. Send enough padding: (max_devices+2)*40 bits total to ensure loopback is
   * captured
   * 4. The command loops back after (n+1)*40 bits where n is the actual chain
   * length
   * 5. Search received data for our exact command pattern
   * 6. When found at offset (n+1)*5, chain length = n
   *
   * @param max_devices Maximum number of devices to probe (default: 8, max:
   * 255)
   * @return Detected chain length (0 = single chip or detection failed, >0 =
   * number of devices)
   *
   * @note This method requires that devices are powered (but don't need to be
   * initialized)
   * @note The command uses a read operation which is safe and doesn't modify
   * device state
   ...
   * chip, default: 0) Specifies which chip in the chain to address
   * @return Result<uint32_t> containing the value or error
   *
   * This method handles both single-chip and daisy-chain modes:
   * - Single chip (daisy_chain_position = 0): Standard 40-bit SPI transaction
   * (5 bytes)
   * - Daisy-chain (daisy_chain_position > 0): Sends command with padding to
   * shift it to the target device position in the chain
   *
   * **Daisy-Chain Transfer Logic:**
   *
   * **CRITICAL: Chain length MUST be known for correct operation**
   * - Chain length is automatically detected on first access if not manually
   * set
   ...
   *   - Receiving requirement: (n-k)*5 bytes to shift response back (datasheet
   * formula)
   *   - Use max() to ensure both requirements are met:
   *     * For k < n/2: (n-k)*5 >= (k+1)*5, so (n-k)*5 dominates
   *     * For k >= n/2: (k+1)*5 > (n-k)*5, so (k+1)*5 dominates
   ...
   * - If `daisy_chain_position > 0` and chain length is unknown
   * (`total_chain_length_ == 0`), chain length is automatically detected on
   * first access
   * - If user has specified a chain length via `SetDaisyChainLength()`, it is
   * verified against auto-detected length on first access
   * - If mismatch is detected, an error is logged and the detected length is
   * used
   * - If detection fails, operation returns false (chain length is required)
   *
   * **Architecture Note:**
   * This SpiCommInterface instance can be shared by multiple TMC5160 drivers on
   * the same SPI bus. Each TMC5160 instance has its own daisy_chain_position_
   * and passes it as a parameter when calling ReadRegister() or
   * WriteRegister().
   *
   * CSN is handled automatically by the SPI hardware or derived class
   * implementation. For daisy-chaining, all chips share the same CSN (tied
   * together).
   */
  Result<uint32_t> ReadRegister(uint8_t address,
   ...
   *
   * **Daisy-Chain Transfer Logic:**
   * Same as ReadRegister() - see ReadRegister() documentation for detailed
   * explanation.
   *
   * **Key Points:**
   ...
   *
   * **Auto-Detection and Verification:**
   * Same as ReadRegister() - chain length is auto-detected on first access if
   * needed, and user-specified length is verified against detected length. If
   * detection fails, operation returns false.
   *
   * **Architecture Note:**
   * This SpiCommInterface instance can be shared by multiple TMC5160 drivers on
   * the same SPI bus. Each TMC5160 instance has its own daisy_chain_position_
...
protected:
  ...
  uint8_t total_chain_length_{
      0}; ///< Total number of devices in daisy chain (0 = unknown/single chip)
  uint8_t user_specified_chain_length_{0}; ///< User-specified chain length (0 =
                                           ///< not specified, >0 = user value)
  bool chain_length_verified_{false}; ///< Flag to track if chain length has
                                      ///< been verified via auto-detection
...
private:
   ...
   * @param daisy_chain_position Position in daisy chain (0 = first chip/single
   * chip)
   * @param context Context string for logging (e.g., "ReadRegister",
   * "WriteRegister")
   * @return Result<void> indicating success or error
   *
   * This function handles:
   * - Auto-detection if daisy_chain_position > 0 and chain length is unknown
   * - Setting chain length to 1 for single chip mode (daisy_chain_position ==
   * 0)
   ...
   *
   * @note This function is defined in the class body, so it's implicitly
   * inline. This allows compiler optimization while maintaining code clarity
   * and avoiding duplication between ReadRegister and WriteRegister.
   */
 ...
 * UART_TXEN. Each byte is transmitted LSB...MSB, highest byte transmitted
 * first.
 *
 ...
 * - SDO_CFG0 (pin 16) → NAO (Next Address Output): Output that connects to next
 * chip's NAI
 * - DIAG0_SWN (pin 26) → SWION (Single Wire I/O Negative): UART single wire
 * interface
 * - DIAG1_SWP (pin 27) → SWIOP (Single Wire I/O Positive): UART single wire
 * interface
 *
 * ## UART Daisy Chaining
 *
 ...
 * - Byte 0: Sync nibble (bits 0-3: 1,0,1,0) + Reserved (bits 4-7) + NODEADDR (8
 * bits)
 ...
 * - Programmable delay after read request before reply (multiples of 8 bit
 * times)
 * - Default: 8 bit times
...
public:
   ...
   * (true=HIGH, false=LOW)
   *
   * @note Multiple TMC5160 instances can share one UartCommInterface on the
   * same UART bus. The node address is passed per transaction via
