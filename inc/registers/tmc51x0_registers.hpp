 ...
 */
namespace Registers {
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage) - Intentional: X-macro pattern
...
enum class EncoderSensitivity : uint8_t {
  ...
  BOTH_EDGES =
      0x03 ///< N channel active on N event activation and de-activation
};
 ...
 * - Bit 0: recalibrate - 1: Zero crossing recalibration during driver disable
 * (via DRV_ENN or via TOFF setting)
 * - Bit 1: faststandstill - Timeout for step execution until standstill
 * detection: 1=Short time (2^18 clocks), 0=Normal time (2^20 clocks)
 * - Bit 2: en_pwm_mode - 1: StealthChop voltage PWM mode enabled (depending on
 * velocity thresholds). Switch from off to on state while in stand-still and at
 * IHOLD=nominal IRUN current, only.
 * - Bit 3: multistep_filt - 1: Enable step input filtering for StealthChop
 * optimization with external step source (default=1)
 * - Bit 4: shaft - 1: Inverse motor direction
 * - Bit 5: diag0_error - (only with SD_MODE=1) 1: Enable DIAG0 active on driver
 * errors: Over temperature (ot), short to GND (s2g). DIAG0 always shows the
 * reset-status, i.e., is active low during reset condition.
 * - Bit 6: diag0_otpw - (only with SD_MODE=1) 1: Enable DIAG0 active on driver
 * over temperature prewarning (otpw)
 * - Bit 7: diag0_stall - (with SD_MODE=1) 1: Enable DIAG0 active on motor stall
 * (set TCOOLTHRS before using this feature) diag0_step - (with SD_MODE=0) 0:
 * DIAG0 outputs interrupt signal, 1: Enable DIAG0 as STEP output (half
 * frequency, dual edge triggered) for external STEP/DIR driver
 * - Bit 8: diag1_stall - (with SD_MODE=1) 1: Enable DIAG1 active on motor stall
 * (set TCOOLTHRS before using this feature) diag1_dir - (with SD_MODE=0) 0:
 * DIAG1 outputs position compare signal, 1: Enable DIAG1 as DIR output for
 * external STEP/DIR driver
 * - Bit 9: diag1_index - (only with SD_MODE=1) 1: Enable DIAG1 active on index
 * position (microstep look up table position 0)
 * - Bit 10: diag1_onstate - (only with SD_MODE=1) 1: Enable DIAG1 active when
 * chopper is on (for the coil which is in the second half of the fullstep)
 * - Bit 11: diag1_steps_skipped - (only with SD_MODE=1) 1: Enable output toggle
 * when steps are skipped in DcStep mode (increment of LOST_STEPS). Do not
 * enable in conjunction with other DIAG1 options.
 * - Bit 12: diag0_int_pushpull - 0: SWN_DIAG0 is open collector output (active
 * low), 1: Enable SWN_DIAG0 push pull output (active high)
 * - Bit 13: diag1_poscomp_pushpull - 0: SWP_DIAG1 is open collector output
 * (active low), 1: Enable SWP_DIAG1 push pull output (active high)
 * - Bit 14: small_hysteresis - 0: Hysteresis for step frequency comparison is
 * 1/16, 1: Hysteresis for step frequency comparison is 1/32
 * - Bit 15: stop_enable - 0: Normal operation, 1: Emergency stop: ENCA_DCIN
 * stops the sequencer when tied high (no steps become executed by the
 * sequencer, motor goes to standstill state)
 * - Bit 16: direct_mode - 0: Normal operation, 1: Motor coil currents and
 * polarity directly programmed via serial interface: Register XTARGET (0x2D)
 * specifies signed coil A current (bits 8..0) and coil B current (bits 24..16).
 * In this mode, the current is scaled by IHOLD setting. Velocity based current
 * regulation of StealthChop is not available in this mode.
 * - Bit 17: test_mode - 0: Normal operation, 1: Enable analog test output on
 * pin ENCN_DCO. IHOLD[1..0] selects the function of ENCN_DCO: 0...2: T120, DAC,
 * VDDH. Hint: Not for user, set to 0 for normal operation!
 * - Bits 18-31: Reserved
 */
union GCONF_Register {
  ...
  struct {
    uint32_t
        recalibrate : 1; ///< Bit 0: 1=Zero crossing recalibration during driver
                         ///< disable (via DRV_ENN or via TOFF setting)
    uint32_t faststandstill : 1; ///< Bit 1: Standstill timeout: 1=Short time
                                 ///< (2^18 clocks), 0=Normal time (2^20 clocks)
    uint32_t en_pwm_mode : 1; ///< Bit 2: 1=StealthChop voltage PWM mode enabled
                              ///< (depending on velocity thresholds). Switch
                              ///< only in stand-still at IHOLD=nominal IRUN.
    uint32_t multistep_filt : 1; ///< Bit 3: 1=Enable step input filtering for
                                 ///< StealthChop optimization with external
                                 ///< step source (default=1)
    uint32_t shaft : 1;       ///< Bit 4: 1=Inverse motor direction
    uint32_t diag0_error : 1; ///< Bit 5: (only with SD_MODE=1) 1=Enable DIAG0
                              ///< active on driver errors (OT, S2G, UV_CP).
                              ///< DIAG0 always shows reset-status.
    uint32_t
        diag0_otpw : 1; ///< Bit 6: (only with SD_MODE=1) 1=Enable DIAG0 active
                        ///< on driver over temperature prewarning (otpw)
    uint32_t
        diag0_stall_step : 1; ///< Bit 7: (SD_MODE=1) 1=Enable DIAG0 active on
                              ///< motor stall (set TCOOLTHRS before using).
                              ///< (SD_MODE=0) 0=DIAG0 outputs interrupt signal,
                              ///< 1=Enable DIAG0 as STEP output (half
                              ///< frequency, dual edge triggered)
    uint32_t
        diag1_stall_dir : 1; ///< Bit 8: (SD_MODE=1) 1=Enable DIAG1 active on
                             ///< motor stall (set TCOOLTHRS before using).
                             ///< (SD_MODE=0) 0=DIAG1 outputs position compare
                             ///< signal, 1=Enable DIAG1 as DIR output
    uint32_t diag1_index : 1; ///< Bit 9: (only with SD_MODE=1) 1=Enable DIAG1
                              ///< active on index position (microstep look up
                              ///< table position 0)
    uint32_t
        diag1_onstate : 1; ///< Bit 10: (only with SD_MODE=1) 1=Enable DIAG1
                           ///< active when chopper is on (for the coil which is
                           ///< in the second half of the fullstep)
    uint32_t diag1_steps_skipped : 1; ///< Bit 11: (only with SD_MODE=1)
                                      ///< 1=Enable output toggle when steps are
                                      ///< skipped in DcStep mode (increment of
                                      ///< LOST_STEPS). Do not enable in
                                      ///< conjunction with other DIAG1 options.
    uint32_t
        diag0_int_pushpull : 1; ///< Bit 12: SWN_DIAG0 output: 0=open collector
                                ///< (active low), 1=push pull (active high)
    uint32_t diag1_poscomp_pushpull : 1; ///< Bit 13: SWP_DIAG1 output: 0=open
                                         ///< collector (active low), 1=push
                                         ///< pull (active high)
    uint32_t small_hysteresis : 1; ///< Bit 14: Step frequency comparison
                                   ///< hysteresis: 0=1/16, 1=1/32
    uint32_t
        stop_enable : 1; ///< Bit 15: 0=Normal operation, 1=Emergency stop:
                         ///< ENCA_DCIN stops the sequencer when tied high (no
                         ///< steps become executed, motor goes to standstill)
    uint32_t direct_mode : 1; ///< Bit 16: 0=Normal operation, 1=Direct motor
                              ///< coil control: XTARGET bits 8..0=coil A
                              ///< current, bits 24..16=coil B current (scaled
                              ///< by IHOLD). Velocity based current regulation
                              ///< of StealthChop not available.
    uint32_t test_mode : 1; ///< Bit 17: 0=Normal operation, 1=Enable analog
                            ///< test output on pin ENCN_DCO. IHOLD[1..0]
                            ///< selects function (0..2: T120, DAC, VDDH). Hint:
                            ///< Not for user, set to 0!
 ...
 *
 * Bit assignments per datasheet:
 * - Bits 2..0: OTPBIT - Selection of OTP bit to be programmed to the selected
 * byte location (n=0..7: programs bit n to a logic 1)
 * - Bits 5..4: OTPBYTE - Set to 00
 * - Bits 15..8: OTPMAGIC - Set to 0xBD to enable programming. A programming
 * time of minimum 10ms per bit is recommended.
 */
union OTP_PROG_Register {
  uint32_t value;

  struct {
    uint32_t otpbit : 3; ///< Bits 2..0: Selection of OTP bit to be programmed
                         ///< (n=0..7: programs bit n to logic 1)
 ...
 * cleared to 0 by default. Programming can only set bits, clearing is not
 * possible.
 *
 * Bit assignments per datasheet OTP memory map:
 * - Bit 7 (otp0.7): otp_TBL - Reset default for TBL (0: TBL=%10 ~3μs, 1:
 * TBL=%01 ~2μs)
 * - Bit 6 (otp0.6): otp_BBM - Reset default for DRVCONF.BBMCLKS (0: BBMCLKS=4,
 * 1: BBMCLKS=2)
 ...
 *                          Factory pre-programmed to 12MHz, differs between
 * ICs!
 */
union OTP_READ_Register {
  ...
  struct {
    ...
    uint32_t
        otp_S2_level : 1;   ///< Bit 5: Reset default for Short detection levels
 ...
 *   Hint: Settings from 1 to 3 will trigger during normal operation due to
 * voltage drop on sense resistor. (Reset Default: OTP 6 or 12)
 ...
 *   Attention: Settings below 6 not recommended at >52V operation – false
 * detection might result (Reset Default: OTP 6 or 12)
 * - Bits 17..16: SHORTFILTER - Spike filtering bandwidth for short detection
 *   0 (lowest, 100ns), 1 (1μs), 2 (2μs), 3 (3μs)
 *   Hint: A good PCB layout will allow using setting 0. Increase value, if
 * erroneous short detection occurs. (Reset Default = %01)
 * - Bit 18: shortdelay - Short detection delay
 *   0=750ns: normal, 1=1500ns: high
 *   The short detection delay shall cover the bridge switching time. 0 will
 * work for most applications. (Reset Default = 0)
 */
union SHORT_CONF_Register {
  uint32_t value;

  struct {
    uint32_t s2vs_level : 4; ///< Bits 3..0: Short to VS detector for low side
                             ///< FETs (4-15, highest to lowest sensitivity)
    uint32_t reserved1 : 4; ///< Bits 7..4: Reserved
    uint32_t s2g_level : 4; ///< Bits 11..8: Short to GND detector for high side
                            ///< FETs (2-15, highest to lowest sensitivity)
    uint32_t reserved2 : 4;   ///< Bits 15..12: Reserved
    uint32_t shortfilter : 2; ///< Bits 17..16: Spike filtering bandwidth
                              ///< (0=100ns, 1=1μs, 2=2μs, 3=3μs)
    uint32_t shortdelay : 1; ///< Bit 18: Short detection delay (0=750ns normal,
                             ///< 1=1500ns high)
...
union DRV_CONF_Register {
  ...
  struct {
    uint32_t bbmtime : 5; ///< Break-Before-Make delay (0..24): 0=100ns ...
                          ///< 16=200ns ... 24=375ns. >24 not recommended, use
                          ///< BBMCLKS instead.
    uint32_t reserved1 : 3; ///< Reserved
    uint32_t
        bbmclks : 4; ///< Digital BBM time in clock cycles (0..15). Each step
                     ///< ≈83ns. Longer of BBMTIME or BBMCLKS prevails.
    uint32_t reserved2 : 4; ///< Reserved
    uint32_t otselect : 2; ///< Overtemperature (OTPW) bridge disable threshold:
                           ///< 0=150°C, 1=143°C, 2=136°C, 3=120°C
    uint32_t drvstrength : 2; ///< Gate driver current: 0=weak, 1=weak+TC,
                              ///< 2=medium, 3=strong
    uint32_t filt_isense : 2; ///< Sense amplifier filter: 0=100ns, 1=200ns,
                              ///< 2=300ns, 3=400ns
...
union OFFSET_READ_Register {
  ...
  struct {
    uint32_t
        phase_b : 8; ///< Phase B offset calibration result (bits 7..0, signed)
    uint32_t
        phase_a : 8; ///< Phase A offset calibration result (bits 15..8, signed)
    uint32_t reserved : 16; ///< Reserved bits
  } bits;
};
 ...
 *   Choose sense resistors so that normal IRUN is 16 to 31 for best microstep
 * performance.
 ...
 *   TPOWERDOWN has expired. The smooth transition avoids a motor jerk upon
 * power down. 0: instant power down 1..15: Delay per current reduction step in
 * multiple of 2^18 clocks
 */
 ...
 *
 * Bit assignments per datasheet:
 * - Bit 0: stop_l_enable - Enable automatic motor stop during active left
 * reference switch input
 * - Bit 1: stop_r_enable - Enable automatic motor stop during active right
 * reference switch input
 * - Bit 2: pol_stop_l - Sets the active polarity of the left reference switch
 * input (0=non-inverted/high active, 1=inverted/low active)
 * - Bit 3: pol_stop_r - Sets the active polarity of the right reference switch
 * input (0=non-inverted/high active, 1=inverted/low active)
 * - Bit 4: swap_lr - Swap the left and the right reference switch input REFL
 * and REFR
 * - Bit 5: latch_l_active - Activate latching of the position to XLATCH upon an
 * active going edge on REFL
 * - Bit 6: latch_l_inactive - Activate latching of the position to XLATCH upon
 * an inactive going edge on REFL
 * - Bit 7: latch_r_active - Activate latching of the position to XLATCH upon an
 * active going edge on REFR
 * - Bit 8: latch_r_inactive - Activate latching of the position to XLATCH upon
 * an inactive going edge on REFR
 * - Bit 9: en_latch_encoder - Latch encoder position to ENC_LATCH upon
 * reference switch event
 * - Bit 10: sg_stop - Enable stop by StallGuard2 (also available in DcStep
 * mode)
 * - Bit 11: en_softstop - Enable soft stop upon a stop event (uses deceleration
 * ramp settings)
 */
union SW_MODE_Register {
  ...
  struct {
    ...
    uint32_t
        pol_stop_r : 1; ///< Sets the active polarity of the right reference
                        ///< switch input (1=inverted, low active)
    uint32_t
        swap_lr : 1; ///< Swap the left and the right reference switch inputs
    uint32_t latch_l_active : 1; ///< Activate latching of the position to
                                 ///< XLATCH upon an active going edge on REFL
    uint32_t
        latch_l_inactive : 1; ///< Activate latching of the position to XLATCH
    ...
    uint32_t
        latch_r_inactive : 1; ///< Activate latching of the position to XLATCH
 ...
 * - Bit 4: event_stop_l - Signals an active stop left condition due to stop
 * switch (ORed to interrupt output)
 * - Bit 5: event_stop_r - Signals an active stop right condition due to stop
 * switch (ORed to interrupt output)
 * - Bit 6: event_stop_sg - Signals an active StallGuard2 stop event (Write '1'
 * to clear, ORed to interrupt output)
 * - Bit 7: event_pos_reached - Signals that the target position has been
 * reached (Write '1' to clear, ORed to interrupt output)
 ...
 * - Bit 11: t_zerowait_active - Signals that TZEROWAIT is active after a motor
 * stop
 * - Bit 12: second_move - Signals that the automatic ramp required moving back
 * in the opposite direction (Write '1' to clear)
 * - Bit 13: status_sg - Signals an active StallGuard2 input from CoolStep
 * driver or DcStep unit
 */
union RAMP_STAT_Register {
  ...
  struct {
    ...
    uint32_t event_pos_reached : 1; ///< Signals that the target position has
                                    ///< been reached
    uint32_t
        velocity_reached : 1; ///< Signals that the target velocity is reached
    uint32_t
        position_reached : 1; ///< Signals that the target position is reached
 ...
 *   (0: N event occurs only when polarities given by pol_N, pol_A and pol_B
 * match) (1: Ignore A and B polarity for N channel event)
 * - Bit 4: clr_cont - Always latch or latch and clear X_ENC upon an N event
 * - Bit 5: clr_once - Latch or latch and clear X_ENC on the next N event
 * following the write access
...
union ENCMODE_Register {
  ...
  struct {
    uint32_t pol_A : 1; ///< Bit 0: Required A polarity for an N channel event
                        ///< (0=neg., 1=pos.)
    uint32_t pol_B : 1; ///< Bit 1: Required B polarity for an N channel event
                        ///< (0=neg., 1=pos.)
    uint32_t pol_N : 1; ///< Bit 2: Defines active polarity of N (0=low active,
                        ///< 1=high active)
    uint32_t
        ignore_AB : 1; ///< Bit 3: Ignore A and B polarity for N channel event
    uint32_t clr_cont : 1; ///< Bit 4: Always latch or latch and clear X_ENC
                           ///< upon an N event
    uint32_t clr_once : 1; ///< Bit 5: Latch or latch and clear X_ENC on the
                           ///< next N event following the write access
    uint32_t
        pos_edge : 1; ///< Bit 6: N channel event sensitivity (positive edge)
    uint32_t
        neg_edge : 1; ///< Bit 7: N channel event sensitivity (negative edge)
    uint32_t clr_enc_x : 1; ///< Bit 8: Clear encoder counter X_ENC upon N-event
    uint32_t latch_x_act : 1; ///< Bit 9: Also latch XACTUAL position together
                              ///< with X_ENC
    uint32_t enc_sel_decimal : 1; ///< Bit 10: Encoder prescaler divisor
                                  ///< (0=binary, 1=decimal)
 ...
 *
 * Bit assignments per datasheet:
 * - Bit 0: n_event - Event detected. To clear the status bit, write with a 1
 * bit at the corresponding position. This bit is ORed to the interrupt output
 * signal.
 * - Bit 1: deviation_warn - Deviation warning. Deviation_warn cannot be cleared
 * while a warning still persists. Set ENC_DEVIATION zero to disable. This bit
 * is ORed to the interrupt output signal.
 */
union ENC_STATUS_Register {
  ...
  struct {
    ...
    uint32_t deviation_warn : 1; ///< Bit 1: Deviation between X_ACTUAL and
                                 ///< X_ENC detected (Write '1' to clear, but
                                 ///< only if warning no longer persists)
 ...
 * Values are signed 9-bit (-256 to 255) as read from MSLUT (not scaled by
 * current).
 *
 * Bit assignments per datasheet:
 * - Bits 8..0: CUR_B (signed) - Actual microstep current for motor phase B
 * (sine wave)
 * - Bits 24..16: CUR_A (signed) - Actual microstep current for motor phase A
 * (co-sine wave)
 */
union MSCURACT_Register {
  ...
  struct {
    int32_t cur_b : 9; ///< Bits 8..0: Actual microstep current phase B (signed,
                       ///< -256 to 255)
    uint32_t reserved1 : 7; ///< Reserved bits (9..15)
    int32_t cur_a : 9;      ///< Bits 24..16: Actual microstep current phase A
                            ///< (signed, -256 to 255)
 ...
 *   %0010...%1111: 2 ... 15
 * - Bits 6..4: hstrt2..hstrt0/TFD[2..0] - Hysteresis start value (chm=0) or
 * fast decay time setting (chm=1) (3 bits) chm=0: %000...%111: Add 1, 2, ..., 8
 * to hysteresis low value HEND chm=1: Fast decay time setting TFD (MSB: fd3)
 * - Bits 10..7: hend3..hend0/HEND/OFFSET - Hysteresis low value (chm=0) or sine
 * wave offset (chm=1) (4 bits) chm=0: %0000...%1111: Hysteresis is -3, -2, -1,
 * 0, 1, ..., 12 chm=1: %0000...%1111: Offset is -3, -2, -1, 0, 1, ..., 12
 * - Bit 11: fd3/TFD[3] - MSB of fast decay time setting (chm=1) or reserved
 * (chm=0)
 * - Bit 12: disfdcc - Fast decay mode (chm=1: disables current comparator usage
 * for termination of fast decay cycle)
 * - Bit 13: Reserved, set to 0
 * - Bit 14: chm - Chopper mode (0=Standard SpreadCycle, 1=Constant off time
 ...
 * - Bit 18: vhighfs - High velocity fullstep selection (enables switching to
 * fullstep when VHIGH is exceeded)
 * - Bit 19: vhighchm - High velocity chopper mode (enables switching to chm=1
 * and fd=0 when VHIGH is exceeded)
 ...
 * - Bit 28: intpol - Interpolation to 256 microsteps (1: extrapolates MRES to
 * 256 microsteps)
 * - Bit 29: dedge - Enable double edge step pulses (1: enables step impulse at
 * each step edge)
 * - Bit 30: diss2g - Short to GND protection disable (1: disables protection)
 * - Bit 31: diss2vs - Short to supply protection disable (1: disables
 * protection)
 */
union CHOPCONF_Register {
  ...
  struct {
    ...
    uint32_t
        chm : 1; ///< Bit 14: Chopper mode (0=SpreadCycle, 1=Constant off time)
 ...
 *
 * Bit assignments per datasheet:
 * - Bits 3..0: semin3..semin0 - Minimum StallGuard2 value for smart current
 * control and smart current enable (4 bits) %0000: smart current control
 * CoolStep off %0001...%1111: 1 ... 15
 * - Bit 4: Reserved, set to 0
 * - Bits 6..5: seup1..seup0 - Current up step width (2 bits)
 *   %00...%11: 1, 2, 4, 8
 * - Bit 7: Reserved, set to 0
 * - Bits 11..8: semax3..semax0 - StallGuard2 hysteresis value for smart current
 * control (4 bits) %0000...%1111: 0 ... 15
...
union COOLCONF_Register {
  ...
  struct {
    uint32_t semin : 4;     ///< Bits 3..0: Minimum StallGuard2 value for smart
                            ///< current control
    ...
    uint32_t semax : 4; ///< Bits 11..8: StallGuard2 hysteresis value for smart
                        ///< current control
    ...
    int32_t sgt : 7; ///< Bits 22..16: StallGuard2 threshold value (signed -64
                     ///< to +63)
 ...
 *
 * Bit assignments per datasheet:
 * - Bits 9..0: SG_RESULT - StallGuard2 result or PWM on time for coil A in
 * standstill (10 bits) Mechanical load measurement: Higher value means lower
 * mechanical load. Value of 0 signals highest load. Temperature measurement: In
 * standstill, shows chopper on-time for motor coil A.
 ...
 * - Bit 14: stealth - StealthChop indicator (1: Driver operates in StealthChop
 * mode)
 * - Bit 15: fsactive - Full step active indicator (1: Driver has switched to
 * fullstep)
 * - Bits 20..16: CS_ACTUAL - Actual motor current / smart energy current (5
 * bits) Actual current control scaling for monitoring smart energy current
 * scaling
 * - Bits 23..21: Reserved, ignore these bits
 * - Bit 24: stallguard - StallGuard2 status (1: Motor stall detected or DcStep
 * stall)
 * - Bit 25: ot - Overtemperature flag (1: Overtemperature limit reached,
 * drivers disabled)
 * - Bit 26: otpw - Overtemperature pre-warning flag (1: Pre-warning threshold
 * exceeded)
 ...
 * - Bit 31: stst - Standstill indicator (1: Motor standstill, occurs 2^20
 * clocks after last step pulse)
 */
union DRV_STATUS_Register {
  uint32_t value;

  struct {
    uint32_t sg_result : 10; ///< Bits 9..0: StallGuard2 result or motor
    ...
    uint32_t cs_actual : 5;  ///< Bits 20..16: Actual motor current / smart
                             ///< energy current
    uint32_t reserved2 : 3;  ///< Bits 23..21: Reserved, ignore these bits (bit
 ...
 * (CS_ACTUAL=31) in stand still Reset default = 30
 ...
 * - Bits 21..20: freewheel1..freewheel0 - Stand still option when motor current
 * setting is zero (I_HOLD=0) (2 bits) %00: Normal operation %01: Freewheeling
 * %10: Coil shorted using LS drivers %11: Coil shorted using HS drivers
 ...
 * pwm_autoscale=1 (1...15) 1: 0.5 increments (slowest regulation) 2: 1
 * increment 4: 2 increments (Reset default) 15: 7.5 increments (fastest
 * regulation)
 * - Bits 31..28: PWM_LIM - PWM automatic scale amplitude limit when switching
 * on (4 bits) Limit for PWM_SCALE_AUTO when switching back from SpreadCycle to
 * StealthChop (Default = 12)
 */
union PWMCONF_Register {
  ...
  struct {
    ...
    uint32_t
        pwm_grad : 8; ///< Bits 15..8: User defined PWM amplitude (gradient)
    uint32_t pwm_freq : 2; ///< Bits 17..16: PWM frequency selection
    uint32_t
        pwm_autoscale : 1; ///< Bit 18: Enable PWM automatic amplitude scaling
    ...
    uint32_t pwm_lim : 4; ///< Bits 31..28: PWM automatic scale amplitude limit
                          ///< when switching on
  } bits;
};
 ...
 * - Bits 24..16: PWM_SCALE_AUTO - 9-bit signed offset added to calculated PWM
 * duty cycle Result of automatic amplitude regulation based on current
 * measurement (signed -255...+255)
 */
union PWM_SCALE_Register {
  ...
  struct {
    ...
    int32_t pwm_scale_auto : 9; ///< Bits 24..16: Result of automatic amplitude
                                ///< regulation (signed -255...+255)
    uint32_t reserved2 : 7; ///< Reserved bits (25..31)
  } bits;
};
