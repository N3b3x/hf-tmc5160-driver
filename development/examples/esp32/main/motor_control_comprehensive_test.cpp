...
bool test_freewheeling_mode() noexcept {
  ...
  if (!handle->driver->motorControl.SetFreewheelingMode(tmc5160::PWMFreewheel::NORMAL)) {
    ESP_LOGE(TAG, "Failed to set freewheeling to NORMAL");
    success = false;
  }
  
  if (!handle->driver->motorControl.SetFreewheelingMode(tmc5160::PWMFreewheel::ENABLED)) {
