...
bool SDCard::init()
{
    ...
    // Note: Display uses SPI bus initialized by SPI.begin(), so we need to check
    // if it's already initialized. If display uses SPI2_HOST, we may need SPI3_HOST for SD card.
    // For now, assume display and SD card can share the same SPI bus with different CS pins.
    ...
    // Use SPI3_HOST to avoid conflict with display (which may use SPI2_HOST)
    ...
    slot_config.host_id = SPI3_HOST;  // Use SPI3_HOST to avoid conflict with display
    ...
    sdmmc_card_t* card;
    ret = esp_vfs_fat_sdspi_mount(s_mount_point, &slot_config, &mount_config, &card);
